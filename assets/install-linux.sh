#!/usr/bin/env bash
#
# Install rusterm's icon + desktop entry on Linux so the GNOME/Ubuntu dock and
...
# running window to its .desktop file via the window's app_id — rusterm sets
# that to "rusterm" (slint::set_xdg_app_id), and this script's StartupWMClass
# matches it.
#
# Usage:
#   ./install-linux.sh [/path/to/rusterm-binary]
# You normally don't need an argument: when run from inside a release package
# (the `rusterm` binary sits next to this script) it is picked up automatically.
# In the source tree it falls back to ./target/release/rusterm.
...
if [ -n "${1:-}" ]; then
    BIN="$1"
elif [ -x "$SCRIPT_DIR/rusterm" ]; then
    BIN="$SCRIPT_DIR/rusterm"
else
    BIN="$SCRIPT_DIR/../target/release/rusterm"
fi
...
if [ ! -x "$BIN" ]; then
    echo "error: rusterm binary not found: $BIN" >&2
    echo "Run this script from the extracted release folder (it sits next to the" >&2
    echo "'rusterm' binary), or pass the binary path as an argument." >&2
    exit 1
fi
...
StartupNotify=true
StartupWMClass=rusterm
EOF
chmod 644 "$APP_DIR/rusterm.desktop"
...
echo "Installed:"
echo "  icon    -> $ICON_DIR/rusterm.png"
echo "  desktop -> $APP_DIR/rusterm.desktop"
echo "  exec    -> $BIN"
echo
